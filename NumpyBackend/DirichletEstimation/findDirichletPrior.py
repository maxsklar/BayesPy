...
startTime = time.time()
parser = OptionParser()
parser.add_option('-s', '--sampleRate', dest='sampleRate', default='1',
                  help='If set to 1, sample every row; if set <1, each row is selected independently on this probability')
parser.add_option('-K', '--numCategories', dest='K', default='2',
                  help='The number of (tab separated) categories that are being counted')
parser.add_option('-M', '--maxCountPerRow', dest='M', type=int, default=sys.maxsize,
                  help='The maximum number of the count per row.  Setting this lower decreases the running time')
parser.add_option("-L", '--loglevel', action="store", dest="loglevel", default='DEBUG',
                  help="don't print status messages to stdout. choose among (ERROR|WARNING|INFO|DEBUG)")
parser.add_option('-H', '--hyperPrior', dest='H', default='0',
